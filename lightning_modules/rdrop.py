...
import torchmetrics
from transformers import BartForConditionalGeneration, get_polynomial_decay_schedule_with_warmup
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.trainer.is_global_zero:
            ...
            self.model.save_pretrained(
                os.path.join(
                    self.model_save_dir,
                    f"rdropmodel-{self.current_epoch:02d}epoch-{self.global_step}steps-{val_loss_mean:.4f}loss-{val_acc_mean:.4f}acc",
                ),
            )
